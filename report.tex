...
Ces données sont ensuite exploitées dans des notebooks (non détaillés ici) pour visualiser les courbes de speedup, l'effet de la taille de données et la répartition des langues, présentés ci-après.
...
Pour disposer d'un Python 3.14 (et exploiter le multi-cœurs via la pile uv), on peut demander à \texttt{uv} de l'installer, créer un environnement virtuel partagé via NFS, puis installer les dépendances du projet :
\begin{verbatim}
uv python install 3.14
uv venv --python 3.14 ~/.venv
uv pip install langdetect
\end{verbatim}
L'environnement \texttt{~/.venv} ainsi créé sera visible depuis toutes les machines partageant le même NFS et aura les mêmes dépendances installées.
...
En cas de crash de certains workers, il est possible de nettoyer les processus Python sur l'ensemble des machines listées dans \texttt{config/nodes.txt} grâce au script shell (à exécuter depuis sa machine locale) :
\begin{verbatim}
